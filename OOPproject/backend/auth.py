...
def get_current_user(request: Request, db: Session = Depends(get_db)):
    ...
    if user.is_banned:
        ...
        raise HTTPException(
            status_code=403, 
            detail=f"Your account has been banned. Reason: {user.ban_reason or 'No reason provided'}"
        )
    
    return user

# Dependency to get current user without ban check (for admin operations)
def get_current_user_no_ban_check(request: Request, db: Session = Depends(get_db)):
    user_id = request.session.get("user_id")
    if not user_id:
        raise HTTPException(status_code=401, detail="Not authenticated")
    user = db.query(models.User).filter(models.User.id == user_id).first()
...
def get_admin_user(user: models.User = Depends(get_current_user_no_ban_check)):
